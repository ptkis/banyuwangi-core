...
@SpringBootApplication(scanBasePackageClasses = [TvShow::class])
@Transactional
class JPASpecificationDSLTest {
        ...
        with(tvShowRepo) {
            hemlockGrove = save(
                TvShow(
                    ...
                    synopsis = "A teenage girl is brutally murdered, " +
                        "sparking a hunt for her killer. But in a town " +
                        "where everyone hides a secret, will they find " +
            ...
            theWalkingDead = save(
                TvShow(
                    ...
                    synopsis = "Sheriff Deputy Rick Grimes leads a group " +
                        "of survivors in a world overrun by the walking " +
                        "dead. Fighting the dead, fearing the living.",
                    releaseDate = "2010",
            ...
            betterCallSaul = save(
                TvShow(
                    ...
                    availableOnNetflix = false,
                    synopsis = "The trials and tribulations of criminal lawyer, " +
                        "Jimmy McGill, in the time leading up to establishing " +
    ...
    fun `Get tv show by isMember`() {
        val shows = tvShowRepo.findAll(
            TvShow::starRatings.isMember(
                theWalkingDead.starRatings.first()
            )
        )
        assertThat(shows, containsInAnyOrder(theWalkingDead))
    }
    ...
    fun `Get tv show by isNotMember`() {
        val shows = tvShowRepo.findAll(
            TvShow::starRatings.isNotMember(
                betterCallSaul.starRatings.first()
            )
        )
    ...
    fun `Find tv shows with and`() {
        val shows = tvShowRepo.findAll(
            TvShow::availableOnNetflix.isFalse()
                and TvShow::releaseDate.equal("2010")
        )
        assertThat(shows, containsInAnyOrder(theWalkingDead))
    }
    ...
    fun `Find tv shows with or`() {
        val shows = tvShowRepo.findAll(
            TvShow::availableOnNetflix.isTrue()
                or TvShow::releaseDate.equal("2010")
        )
        assertThat(shows, containsInAnyOrder(hemlockGrove, theWalkingDead))
    }
    ...
    fun `Test Join`() {
        val shows = tvShowRepo.findAll(
            where {
                equal(
                    it.join(TvShow::starRatings)
                        .get(StarRating::stars),
                    2
                )
            }
        )
